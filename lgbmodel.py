...
def create_dt(is_train=True, nrows=None, first_day=1200):
    prices = pd.read_csv("../M5Walmart/data/sell_prices.csv", dtype=PRICE_DTYPES)
    # print('Prices')
    #
    ...
    cal = pd.read_csv("../M5Walmart/data/calendar.csv", dtype=CAL_DTYPES)
    ...
    # calendar_prices = cal[['wm_yr_wk', 'month', 'year']]
    ...
    # # by month mean
    # # by year mean
    # prices['price_momentum'] = prices['sell_price'] / prices.groupby(['store_id', 'item_id'])[
    #         'sell_price'].transform(lambda x: x.shift(1))
    # prices['price_momentum_m'] = prices['sell_price'] / prices.groupby(['store_id', 'item_id', 'month'])[
    #         'sell_price'].transform('mean')
    # prices['price_momentum_y'] = prices['sell_price'] / prices.groupby(['store_id', 'item_id', 'year'])[
    #         'sell_price'].transform('mean')
    ...
    dt = pd.read_csv("../M5Walmart/data/sales_train_evaluation.csv",
...
fake_valid_inds = np.random.choice(len(X_train), 10000000)
...
for i in range(0, 28):
    ...
    #te.loc[te.date == day, "sales"] = round(m_lgb.predict(tst))  # round the sale
    #te.loc[te.date == day, "sales"] =np.around(m_lgb.predict(tst))
...
te_sub.to_csv("submission_eval.csv",index=False)
